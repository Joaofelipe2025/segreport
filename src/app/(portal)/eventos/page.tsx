        ...
        <div className="mx-auto max-w-[1400px] px-4 py-10 sm:py-14 lg:px-8 lg:py-20">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-2xl">
              <span className="inline-flex items-center gap-2 rounded-full bg-forest-700 px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-[0.1em] text-lime-400">
            ...
            <Link
              href="/contato"
              className="inline-flex items-center gap-2 rounded-lg bg-lime-400 px-5 py-3 text-sm font-semibold text-forest-800 transition-colors hover:bg-lime-500"
            >
  ...
  return (
    <article className="flex flex-wrap items-center gap-5 rounded-xl border border-hairline bg-white p-5 transition-shadow hover:shadow-[0_10px_30px_rgba(14,31,20,0.07)]">
      <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-lg bg-paper">
        <span className="text-xl font-semibold leading-none text-ink">{day}</span>
        <span className="mt-0.5 text-[10px] font-semibold tracking-wide text-ink-3">
          {month}
        </span>
      </div>
      ...
      <div className="min-w-0 flex-1">
        <span
          className={`inline-block rounded px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.06em] ${KIND_STYLES[event.kind]}`}
        >
          {EVENT_KIND_LABELS[event.kind]}
        </span>

        <h3 className="mt-2 text-base font-semibold leading-snug text-ink lg:text-lg">
          {event.title}
        </h3>
      ...
      <Link
        href="/contato"
        className="shrink-0 rounded-lg border border-hairline px-5 py-2.5 text-xs font-semibold text-ink-2 transition-colors hover:border-forest-700 hover:bg-forest-800 hover:text-white"
      >
        Saiba mais
      </Link>
    </article>
